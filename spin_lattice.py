...
class SpinLattice:
    ...
    def __init__(
        ...
        J=1.0,
        nsweeps=10000,
        with_auto_correlation_times_condition=False,
    ):
        self.lattice_dimensions = lattice_dimensions
        self.spin_lattice = np.ones(
            (lattice_dimensions, lattice_dimensions), dtype=float
        )
        ...
        self.itrial_1 = 0
        self.jtrial_1 = 0
        self.with_auto_correlation_times_condition = (
            with_auto_correlation_times_condition
        )
    ...
    def calculate_total_energy(self):
        ...
        for row in range(lattice_dimensions):
            for column in range(lattice_dimensions):
                ...
                # energies.
                total_energy += (
                    self.calculate_spin_configuration_energy(row, column) / 2
                )
    ...
    def select_candidate_state_kawasaki(self):
        ...
        while True:
            ...
            if (spin_lattice[itrial, jtrial] != spin_lattice[itrial_1, jtrial_1]) and (
                itrial != itrial_1 and jtrial != jtrial_1
            ):
                break
    ...
    def nearest_neighbours_check_kawasaki(self):
        ...
        if (spin_1_position in second_spin_neighbours) or (
            spin_2_position in first_spin_neighbours
        ):
            return True
    ...
    def calculate_energy_difference_kawasaki(self):
        ...
        if self.nearest_neighbours_check_kawasaki():
            ...
            energy_current_state = self.calculate_spin_configuration_energy(
                itrial, jtrial
            ) + self.calculate_spin_configuration_energy(itrial_1, jtrial_1)

            energy_candidate_state = -self.calculate_spin_configuration_energy(
                itrial, jtrial
            ) - self.calculate_spin_configuration_energy(itrial_1, jtrial_1)
    ...
    def animate_ising_model_glauber(self):
        ...
        for sweep in range(nsweeps):
            ...
            if not np.mod(sweep, 10) and sweep > 400:
                update_string = "Current sweep: {}, Energy Difference: {}".format(
                    sweep, energy_difference
                )
                print(update_string)
    ...
    def calculate_error(self, observable_data):
        ...
        variance_xi_bar = (
            np.sum(np.square(xi_bar - mean_xi_bar))
            * (length_observable_data - 1)
            / length_observable_data
        )
    ...
    def calculate_scaled_specific_heat_capacity(self, energies):
        ...
        return (mean_squared_energy - mean_energy_squared) / (
            (lattice_dimensions**2) * (temperature**2)
        )
    ...
    def calculate_susceptibility(self, magnetisations):
        ...
        return (mean_squared_magnetisations - squared_mean_magnetisations) / (
            (lattice_dimensions**2) * temperature
        )
    ...
    def calculate_observables_glauber_without_auto_correlation_times(self):
        ...
        for sweep in range(nsweeps + 101):
            ...
            if sweep > 100:
                with open(
                    "Spins_Data/Spins_Data_"
                    + str(np.round(temperature, 2))
                    + "/spins_glauber_"
                    + str(np.round(temperature, 2))
                    + "_sweeps="
                    + str(sweep)
        ...
        mean_absolute_magnetisation_error = self.calculate_error(
            np.abs(np.array(magnetisations))
        )
        scaled_specific_heat_capacity = self.calculate_scaled_specific_heat_capacity(
        ...
        mean_total_energy_error_per_spin = mean_total_energy_error / (
            lattice_dimensions**2
        )
        mean_absolute_magnetisation_per_spin = mean_absolute_magnetisation / (
            lattice_dimensions**2
        )
        mean_absolute_magnetisation_error_per_spin = (
            mean_absolute_magnetisation_error / (lattice_dimensions**2)
        )
        absolute_magnetisations_per_spin = np.abs(np.array(magnetisations)) / (
            lattice_dimensions**2
        )
        ...
        absolute_magnetisations_per_spin_dict = {
            "Absolute Magnetisations Per Spin": absolute_magnetisations_per_spin
        }
        absolute_magnetisations_per_spin_values = pd.DataFrame.from_dict(
            absolute_magnetisations_per_spin_dict
        )
        absolute_magnetisations_per_spin_values.to_csv(
            "Absolute_Magnetisations_Per_Spin/absolute_magnetisations_per_spin_for_temperaterature_"
            + str(np.round(temperature, 2))
            + ".txt"
        )

        return (
            mean_total_energy_per_spin,
            mean_total_energy_error_per_spin,
            mean_absolute_magnetisation_per_spin,
            mean_absolute_magnetisation_error_per_spin,
            scaled_specific_heat_capacity_per_spin,
    ...
    def calculate_observables(self, dynamical_rule):
        ...
        with_auto_correlation_times_condition = (
            self.get_with_auto_correlation_times_condition()
        )

        if with_auto_correlation_times_condition:
            auto_correlation_times = pd.read_csv(
                "Auto_Correlation_Times/auto_correlation_times.csv"
            )["Auto Correlation Times"].to_numpy()
            ...
            if dynamical_rule == "Glauber":
                if with_auto_correlation_times_condition:
                    (
                        ...
                        scaled_specific_heat_capacity_per_spin,
                        susceptibility_per_spin,
                    ) = self.calculate_observables_glauber_with_auto_correlation_times()
                else:
                    (
                        mean_total_energy_per_spin,
                        mean_total_energy_error_per_spin,
                        ...
                        self.calculate_observables_glauber_without_auto_correlation_times()
                    )

                mean_total_energies_per_spin.append(mean_total_energy_per_spin)
                mean_total_energies_errors_per_spin.append(
                    mean_total_energy_error_per_spin
                )
                mean_absolute_magnetisations_per_spin.append(
                    mean_absolute_magnetisation_per_spin
                )
                mean_absolute_magnetisations_errors_per_spin.append(
                    mean_absolute_magnetisation_error_per_spin
                )
                scaled_specific_heat_capacities_per_spin.append(
                    scaled_specific_heat_capacity_per_spin
                )
        ...
        if dynamical_rule == "Glauber":
            observables_dictionary = {
                "Temperatures": np.round(np.linspace(1.0, 3.54, 21), 2),
            ...
            observables_dataframe.to_csv(
                "Glauber_Observables/glauber_observables_data.csv"
            )

        elif dynamical_rule == "Kawasaki":
            observables_dictionary = {
                "Temperatures": np.linspace(1.0, 3.54, 21),
                "Mean Total Energies Per Spin": mean_total_energies_per_spin,
                "Mean Total Energies Errors Per Spin": mean_total_energies_errors_per_spin,
                "Scaled Specific Heat Capacities Per Spin": scaled_specific_heat_capacities_per_spin,
            }
            observables_dataframe = pd.DataFrame.from_dict(observables_dictionary)
            observables_dataframe.to_csv(
                "Kawasaki_Observables/kawasaki_observables_data.csv"
            )
    ...
    def plot_mean_energy_against_temperature(self, dynamical_rule):
        ...
        if dynamical_rule == "Glauber":
            observables_data = pd.read_csv(
                "Glauber_Observables/glauber_observables_data.csv"
            )
            temperatures = observables_data["Temperatures"]
            mean_energies_per_spin = observables_data["Mean Total Energies Per Spin"]
            mean_energies_errors_per_spin = observables_data[
                "Mean Total Energies Errors Per Spin"
            ]
        elif dynamical_rule == "Kawasaki":
            observables_data = pd.read_csv(
                "Kawasaki_Obervables/kawasaki_observables_data.csv"
            )
            temperatures = observables_data["Temperatures"]
            mean_energies_per_spin = observables_data["Mean Total Energies Per Spin"]
            mean_energies_errors_per_spin = observables_data[
        ...
        plt.cla()
        plt.title(
            "Mean Energy Per Spin Against Temperature For "
            + dynamical_rule
            + " Dynamics"
        )
        ...
        plt.errorbar(
            temperatures, mean_energies_per_spin, yerr=mean_energies_errors_per_spin
        )
    ...
    def plot_absolute_magnetisation_against_temperature(self):
        ...
        temperatures = observables_data["Temperatures"]
        absolute_mean_magnetisations_per_spin = observables_data[
            "Mean Absolute Magnetisations Per Spin"
        ]
        absolute_mean_magnetisations_errors_per_spin = np.abs(
            observables_data["Mean Absolute Magnetisations Errors Per Spin"]
        )

        plt.cla()
        plt.title(
            "Absolute Mean Magnetisation Per Spin Against Temperature For Glauber Dynamics"
        )
        plt.xlabel("Temperature, T")
        plt.ylabel(
            "Absolute Mean Magnetisation Per Spin, $\displaystyle\langle |M| \\rangle$"
        )
        plt.errorbar(
            temperatures,
            absolute_mean_magnetisations_per_spin,
            yerr=absolute_mean_magnetisations_errors_per_spin,
        )
    ...
    def plot_scaled_specific_heat_against_temperature(self, dynamical_rule):
        ...
        if dynamical_rule == "Glauber":
            ...
            temperatures = observables_data["Temperatures"]
            scaled_specific_heat_capacities_per_spin = observables_data[
                "Scaled Specific Heat Capacities Per Spin"
            ]
            mean_energies_errors_per_spin = observables_data[
                "Mean Total Energies Errors Per Spin"
            ]
        ...
        plt.cla()
        plt.title(
            "Scaled Specific Heat Capacity against Temperature for "
            + dynamical_rule
            + " Dynamics"
        )
        plt.xlabel("Temperature, $T$")
        ...
        plt.errorbar(
            temperatures,
            scaled_specific_heat_capacities_per_spin,
            yerr=mean_energies_errors_per_spin,
        )
    ...
    def plot_auto_correlation_function_of_magnetisation_per_spin_against_sweeps(
        self, tau
    ):
        """
        ...
        plt.cla()
        plt.title(
            "Auto-Correlation Function of Magnetisation Per Spin Against the Number of Sweeps For Glauber Dynamics"
        )
        plt.xlabel("Sweeps")
        plt.ylabel(
            "Auto-Correlation Function of Magnetisation Per Spin, $\displaystyle\langle |M| \\rangle$"
        )
        plt.scatter(sweeps, self.auto_correlation_time_exponential(sweeps, tau))
        plt.savefig(
            "Graphs/Auto_Correlation_Function_of_Magnetisations_Per_Spin_Against_Sweeps/magnetisation_against_sweeps_at_temperature_"
            + str(np.round(temperature, 2))
            + ".png"
        )
    ...
    def calculate_correlation_function(self, temperature):
        ...
        absolute_magnetisations_per_spin_data = pd.read_csv(
            "Absolute_Magnetisations_Per_Spin/absolute_magnetisations_per_spin_for_temperaterature_"
            + str(round(temperature, 2))
            + ".txt"
        )
        absolute_magnetisations_per_spin = absolute_magnetisations_per_spin_data[
            "Absolute Magnetisations Per Spin"
        ]
        mean_absolute_magnetisation = np.mean(
            np.array(absolute_magnetisations_per_spin)
        )
        correlation_values = []
        for sweep in range(1, nsweeps):
            correlation_values.append(
                (
                    absolute_magnetisations_per_spin[sweep - 1]
                    * absolute_magnetisations_per_spin[sweep]
                )
                - mean_absolute_magnetisation**2
            )
    ...
    def calculate_auto_correlation_time(self):
        ...
        for temperature in np.linspace(1.00, 3.54, 21):
            ...
            popt = curve_fit(
                self.auto_correlation_time_exponential, sweeps, correlation_values
            )[0]
            tau = int(abs(1 / popt))
            self.plot_auto_correlation_function_of_magnetisation_per_spin_against_sweeps(
                tau
            )
            auto_correlation_times.append(tau)

        auto_correlation_times_dict = {"Auto Correlation Times": auto_correlation_times}
        auto_correlation_times_data = pd.DataFrame.from_dict(
            auto_correlation_times_dict
        )
        auto_correlation_times_data.to_csv(
            "Auto_Correlation_Times/auto_correlation_times.csv"
        )
